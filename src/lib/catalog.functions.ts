...
export const getSpecialties = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const { data, error } = await supabase
    .from("specialties")
    .select("id,name,slug,description,image_url,price_hint")
    .eq("active", true)
    .order("sort_order");
...
export const getCakeOptions = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const [bases, fillings, sizes] = await Promise.all([
    supabase.from("cake_bases").select("id,name,description").eq("active", true).order("sort_order"),
    supabase
      .from("cake_fillings")
