...
import { buildStandardMessage, buildWhatsAppLink } from "@/lib/whatsapp";
...
export const Route = createFileRoute("/prenota")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Prenota dolci artigianali napoletani e ritira in pasticceria." },
...
function BookingStandardPage() {
  ...
  const canSubmit = useMemo(
    () => cart.length > 0 && name.trim() && phone.trim() && pickupDate,
    [cart, name, phone, pickupDate],
  );
  ...
  function addToCart(itemName: string) {
    ...
    setCart((c) => {
      ...
      if (existing) return c.map((x) => (x.name === itemName ? { ...x, quantity: x.quantity + 1 } : x));
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="rounded-2xl bg-card p-6 ring-1 ring-border">
            ...
            <div className="mt-4 flex gap-2">
              <Select value={addItem} onValueChange={(v) => { addToCart(v); setAddItem(""); }}>
                ...
                {cart.map((it) => (
                  <li key={it.name} className="flex items-center justify-between gap-3 py-3">
                    ...
                    <div className="flex items-center gap-2">
                      <Button type="button" size="icon" variant="outline" onClick={() => changeQty(it.name, -1)}>
                      ...
                      <Button type="button" size="icon" variant="ghost" onClick={() => changeQty(it.name, -it.quantity)}>
